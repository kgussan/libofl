...
static uint32_t GLTB_TIME_EPSILON = 10;

static GLuint  gltb_lasttime;
static float   gltb_lastposition[3];

static float   gltb_angle = 0.0f;
static float   gltb_axis[3];
static float   gltb_transform[4][4];

static GLuint    gltb_width;
static GLuint    gltb_height;

static int32_t	gltb_button = -1;
static bool gltb_tracking =	false;
static bool gltb_animate =	true;


static void _gltbPointToVector(int x, int y, int width, int height, float v[3])
{
  float d, a;
...
void _gltbStopMotion(int button, unsigned time)
{
	(void)button;
	assert(gltb_button != -1);
